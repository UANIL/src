...
#Date Last Modified: 16 April 2018
...
#Purpose: This program was built for the Network Intrusion Lab in Cyber Hall at the University of 
		 #Alabama. It's purpose is to help Dr. Diana Dolliver and Dr. Ghazi-Tehrani with their 
		 #research into the Dark Web.
...
#intialize file pointers to None
...
def read_text(Tables): # - HIGH PRIORITY
	...
	if bitcoin_address_lineCount is None: #file doesn't exist -- going to create one
	...
	#	variable for telling the user how progress is going
		...
		insertSeen = False # These 5 variables are used for correctly parsing individual values in a table
		valuesSeen = False
		ignoreInput = False
		...
		while True:
			...
			if("VALUES" in bufferText): #Table Structure: "INSERT INTO `tablename` VALUES "
				valuesSeen = True
			...
			if (char == '"' and trackChar and prevChar != "\\"):
...
def run():
	...
	while(command is not "q"):
		...
		command = get_command(commands, command)
